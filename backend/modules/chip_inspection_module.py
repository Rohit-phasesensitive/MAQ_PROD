# modules/chip_inspection_module.py - Chip Inspection Backend Module
...
chip_inspection_router = APIRouter()
...
def init_chip_inspection_tables():
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS chip_inspections (
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_chip_inspections_status 
                ON chip_inspections(status)
            """)
            
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_chip_inspections_created_at 
                ON chip_inspections(created_at)
            """)
            
            conn.commit()
            print("✅ Chip inspection tables initialized")
            
    except Exception as e:
        print(f"❌ Chip inspection table initialization error: {e}")

# Initialize tables on module load
init_chip_inspection_tables()

# File handling utilities
UPLOAD_DIR = "uploads/chip_inspections"
...
# API Endpoints
@chip_inspection_router.get("/status")
async def chip_inspection_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                ...
                FROM chip_inspections
            """)
            stats = cursor.fetchone()
            
            return {
                "module": "chip_inspection",
                "status": "operational",
...
@chip_inspection_router.post("/save")
async def save_inspection(
    ...
    """Save chip inspection with manufacturing order integration"""
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO chip_inspections 
                (inspection_id, operator, chip_number, wafer_id, notes, status, 
                 image_path, image_filename, manufacturing_order_number, 
                 purchase_order_number, device_type, created_by)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                inspection_id, operator, chip_number, wafer_id, notes, status,
                image_path, image_filename, manufacturing_order_number,
                purchase_order_number, device_type, current_user['user_id']
            ))
            
            conn.commit()
        
        # Update manufacturing order progress if MO is specified
        if manufacturing_order_number:
            try:
                from modules.manufacturing_orders_module import update_manufacturing_order_progress
                update_manufacturing_order_progress(manufacturing_order_number, status)
            except ImportError:
                print("Manufacturing orders module not available for progress update")
        ...
        log_action(
            ...
            'inspection_save', 
            'chip_inspection',
            f"Saved inspection for chip {chip_number}, wafer {wafer_id}"
        )
        
        # Notify users
        await notify_module_users('chip_inspection', 'inspection_saved', {
            'operator': operator,
...
@chip_inspection_router.get("/inspections")
async def get_inspections(
    ...
    try:
        with get_db_connection() as conn:
            ...
            query = f"""
                SELECT inspection_id, operator, chip_number, wafer_id, notes, status,
...
@chip_inspection_router.put("/update-status")
async def update_inspection_status(
    ...
    try:
        ...
        await notify_module_users('chip_inspection', 'status_updated', {
...
@chip_inspection_router.get("/image/{inspection_id}")
async def get_inspection_image(
    inspection_id: str,  # Changed from int to str (UUID)
    current_user: dict = Depends(get_current_user)
):
    """Get inspection image by inspection_id (UUID)"""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT image_path, image_filename FROM chip_inspections WHERE inspection_id = %s",
                (inspection_id,)  # Use inspection_id instead of id
            )
...
@chip_inspection_router.post("/generate-report")
async def generate_inspection_report(
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            query = f"""
                SELECT operator, chip_number, wafer_id, notes, status, created_at
                FROM chip_inspections
                {where_clause}
                ORDER BY created_at DESC
            """
        ...
        for inspection in inspections:
            pdf.cell(25, 6, str(inspection['chip_number'])[:10], 1)
            pdf.cell(25, 6, str(inspection['wafer_id'])[:10], 1)
        ...
        pdf_filename = f"chip_inspection_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
...
@chip_inspection_router.get("/statistics")
async def get_inspection_statistics(current_user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Total inspections
            cursor.execute("SELECT COUNT(*) FROM chip_inspections")
...
# Export router
__all__ = ['chip_inspection_router']
